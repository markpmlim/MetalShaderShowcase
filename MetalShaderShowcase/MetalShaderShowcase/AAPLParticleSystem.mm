...
- (instancetype)initWithDevice:(id <MTLDevice>)device
{
    ...
    if (self)
    {
        ...
        for (unsigned int i = 0; i < NUM_PARTICLES; i++)
        {
            ...
            // one.
            _initialDirection[i*3+0] = d_0_x / length;
            _initialDirection[i*3+1] = d_0_y / length;
            _initialDirection[i*3+2] = d_0_z / length;
            
        }

        _initial_direction_buffer = [device newBufferWithBytes:_initialDirection
                                                        length:initialDirectionDataSize
        ...
        _birth_offsets_buffer = [device newBufferWithBytes:_birthOffsets
                                                    length:birthOffsetsDataSize
                                                   options:MTLResourceOptionCPUCacheModeDefault];
