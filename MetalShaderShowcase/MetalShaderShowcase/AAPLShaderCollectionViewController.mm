...
// An instance of this is created when Shader Scene object is instantiated in storyboard.
...
- (void) viewDidLoad {
	[super viewDidLoad];
	// These png images would be loaded and display with a cell
	shaderImages = [NSArray arrayWithObjects:
						@"PhongShader",
						@"WoodShader",
						@"FogShader",
						@"CelShader",
				  ...
				  sender:(id)sender
{
	if ([segue.identifier isEqualToString:@"showShader"])
	{
		...
		// A new Renderer object is instantiated every time.
		switch (index) {
			case Phong:
			...
			case ParticleSystem:
				...
				break;
			default:
				_renderer = [[AAPLRenderer alloc] initWithName:@"Default Shader"
												  vertexShader:@"phong_vertex"
												fragmentShader:@"phong_fragment"
														  mesh:_teapotMesh];
				break;
				   ...
				   cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
	...
	shaderImageView.image = [UIImage imageNamed:[shaderImages
...
// overridden UIViewController method.
								 ...
								 duration:(NSTimeInterval)duration
{
	...
	[super willRotateToInterfaceOrientation:orientation
								   duration:duration];
}
