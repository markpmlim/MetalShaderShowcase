/*
 ...
 This is a common implementation of non-photorealistic rendering technique.
 This effect is accomplished by breaking down the diffuse component into
 3 different shades and the specular into only one. The diffuse color is decided
 upon by calculating the angle between the normal and the light vector,
 and setting three different angles as the boundaries of these regions.
 It is similarly done with the specular component with only one angle
 between the viewer and the reflection vector determining the boundary.
 */
...
// assumes the light's position is in world space
...
vertex ColorInOut
cel_shading_vertex(device packed_float3*        vertices    [[ buffer(0) ]],
...
fragment half4
cel_shading_fragment(ColorInOut in [[stage_in]])
{
    half4 color;
    ...
    {
        specular_factor = smoothstep(0.5f - epsilon, 0.5f + epsilon,
                                     specular_factor);
    }
