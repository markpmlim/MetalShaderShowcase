...
- (void) viewDidLoad {
    ...
    dispatch_source_set_event_handler(_displaySource, ^(){
        ...
        [weakSelf renderPass];
        // The following statement is necessary to set the "_layerSizeDidUpdate" ivar
...
- (void) loadAssets:(id <MTLDevice>)device {
    ...
    for (int i=0; i < shaderNames.count; i++) {
        NSURL *url = [mainBndl URLForResource:shaderNames[i]
                                withExtension:@"png"];
        MSSImageFile *imageFile = [[MSSImageFile alloc] initWithURL:url];
...
- (NSCollectionViewItem *) collectionView:(NSCollectionView *)cv
    ...
    // for a .nib file named "MSCollectionView".
