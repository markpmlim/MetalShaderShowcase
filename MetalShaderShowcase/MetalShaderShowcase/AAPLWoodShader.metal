/*
 ...
 Abstract:
 A shader using random noise to create a wood texture for the Metal Shader Showcase.
 This is an example of a 3D procedural texture based shader.
 The wood texture is accomplished by making rings of two different colors and
 using perlin noise to add some variation to the rings.
 */
...
float smoothNoise(float x, float y, float z)
{
    ...
    // Get the fractional remainder of x, y, and z; could fract() be used?
    float fractX = x - intX;
    float fractY = y - intY;
    float fractZ = z - intZ;

    // Get first whole number before the given position
    int x1 = (intX + NOISE_DIM) % NOISE_DIM;
    int y1 = (intY + NOISE_DIM) % NOISE_DIM;
    int z1 = (intZ + NOISE_DIM) % NOISE_DIM;

    // Get the number after the given position
...
// The verter data of position, normals, matrices etc. are passed as parameters to the vertex function
vertex ColorInOut
wood_vertex(device packed_float3*       vertices    [[ buffer(0) ]],
            ...
            unsigned int                vid         [[ vertex_id ]])
{
    ...
    out.position = mvp_matrix * vertex_position_modelspace;     // clip space
