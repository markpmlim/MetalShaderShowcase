...
float getFogComponent(float objectDistance)
{
    ...
    fogComponent = 1.0f - clamp(fogComponent,
                                0.0f, 1.0f);

    return fogComponent;    // [0.0, 1.0]
}

// Calculate the percieved color with the fog given the object's distance and color.
float4 calculateColorWithFog(float objectDistance, float3 objectColor)
{
    ...
    float4 colorWithFog;
    colorWithFog.xyz = mix(objectColor, fogColor,
                           fogComponent);
    colorWithFog.w = 1.0f;
...
vertex ColorInOut
fog_vertex(device packed_float3*        vertices    [[ buffer(0) ]],
           ...
           unsigned int                 vid         [[ vertex_id ]])
{
    ...
    // Transform the incoming position in object space to clip space.
    ...
    // Calculate the direction of the light from surface to light source in camera space.
    ...
    // Calculate the distance to the object which is used for how much fog obscures the object
...
fragment half4
fog_fragment(ColorInOut in [[stage_in]])
{
    half4 color;
    ...
    color = half4( calculateColorWithFog(in.distance_to_object,
                                         object_color.xyz));
