...
vertex ColorInOut
sphere_map_vertex(device packed_float3*         vertices    [[ buffer(0) ]],
                  ...
                  unsigned int                  vid         [[ vertex_id ]])
{
    ...
    // Compute the incident ray in view space.
    out.position_cameraspace = normalize( (model_view_matrix * vertex_position_modelspace).xyz );

    return out;
}

// Sphere map fragment shader function. It uses a texture that represents a spherical mirror
// to lookup the color reflected by the environment in any direction seen by the viewer.
fragment half4
sphere_map_fragment(ColorInOut in [[stage_in]],
                    texture2d<float>  tex2D [[ texture(0) ]])
{
    ...
    float3 rVec = reflect(in.position_cameraspace,
                          normalize(in.normal_cameraspace));
    float3 viewVec(0, 0, 1);        // view vector - direction towards camera.
    float3 sNormal = rVec + viewVec;

    // normalize the sphere's normal.
    ...
    // Calculate the final color by mixing the phong and the reflective colors
    float4 final_color;
    final_color.rgb = mix(phong_color, reflectiveColor,
                          reflectiveFactor);
    final_color.a = 1.0f;
