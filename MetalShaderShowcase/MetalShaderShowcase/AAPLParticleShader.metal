/*
 ...
 Particles are a common effect implemented in many 3D applications.
 Each particle's initial random direction and birth offset is passed into the vertex shader.
 It uses these values to calculate the particles position based on the current time.
 Then the fragment shader uses these points and colors them as circles that fade out at the edges.
 KIV. compute shader
 */
...
constant float3 x_0 = float3(0.0, 0.1, 0.0);    // initial position of each particle.


// Particle vertex shader function
// The data of vertices, normals, matrices etc. are passed as parameters to the vertex function
vertex ColorInOut
particle_vertex(device packed_float3*       initialDirection    [[ buffer(0) ]],
...
fragment half4
particle_fragment(ColorInOut in [[stage_in]],
